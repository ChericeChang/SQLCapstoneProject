...
Return the facid, facility totalname, member cost, and monthly maintenance of the
...
select
	  count(bookid) as 'Number of times member used a tennis court',
	  f.name,
...
select table1.name as 'facility name',
       concat(m.firstname, ' ', m.surname) as 'member name',
	   table1.bookingcost as 'booking cost'
...
from (
	Select f.name,
    	...
    	from
 		Bookings as b
   	    left join Facilities as f
	    on b.facid = f.facid) table1

left join Members as m
on m.memid = table1.memid

where table1.starttime between '2012-09-14 00:00:00' and '2012-09-14 23:59:59'
order by bookingcost desc
...
Select m1.memid, m1.surname, m1.firstname, m2.surname as 'recommendedby_surname', m2.firstname as 'recommendedby_firstname'
from Members as m1
...
using (facid)
where b.memid > 0
group by month(b.starttime), facid